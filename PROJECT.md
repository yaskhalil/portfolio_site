...
## Status
Live. Hero title overlays the ASCII mesh (above the fold) with a light panel; TechnicalMatrix has the ship-vs-asteroids tree defense game (asteroids are section-anchored, scroll with the page); TerminalCTA is an interactive shell (type `help`); scrollspy + keyboard nav; OG image + JSON-LD. Projects: Temple (FlyGPlot + KSG split-GAL4 proposal), VT (STP-Diff, DOI 10.13140/RG.2.2.35274.32965), Talii, La-Unica restaurant (commissioned design + operations), Real Estate Startup (ML lead, NDA), HoneyRuns, Diggeridoos — bullets are deliberately explanatory (problem → approach → outcome). Blog list mirrors what is published on Medium (@yaskhalil2006) and Substack (@yaseenkhalil), including "Building in Public When You're Still Figuring It Out" (2026-07-30); Ditch section removed. Design system in docs/SYSTEM_DESIGN.md is the single source of truth.
