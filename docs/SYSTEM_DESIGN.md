...
7. **TerminalCTA** (`#research`) — terminal window: `cat ./status.txt` → "Seeking a new challenge.", `ls ./contact` → email, github, linkedin, x, medium, substack, resume links, then an interactive prompt: type a command and Enter. Commands/aliases: email, github, linkedin, x/twitter, medium, substack, resume/cv, status, clear, help/?/ls. Unknown → `bash: <cmd>: command not found. Try 'help'.` URL commands open in a new tab (mailto/resume navigate in place). Clicking the terminal focuses the input.
...
- **Metadata**: `app/opengraph-image.tsx` generates a dark terminal-card OG image (edge runtime, dynamic); JSON-LD Person schema in root layout (github/linkedin/x/medium/substack sameAs).
- **Resume**: `public/resume.pdf` — currently a copy of Documents/Yaseen Resume.pdf (Mar 2025); replace with the current version before sharing widely.
